...
public sealed class SendVerifyEmailDomainEvent(
    IFluentEmail fluentEmail,
    IOutboxSendEmailRepository outboxSendEmailRepository
    ) : INotificationHandler<AuthDomainEvent>
{
    public async Task Handle(AuthDomainEvent notification, CancellationToken cancellationToken)
    {
        string body = CreateConfirmEmailBody(notification.EmailConfirmCode.ToString());
        ...
        await outboxSendEmailRepository.AddAsync(outboxSendEmail, cancellationToken);
        ...
        if (sendResponse.Successful)
        {
            ...
            await outboxSendEmailRepository.UpdateAsync(outboxSendEmail, cancellationToken);
        }
    }
